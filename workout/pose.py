...
class PoseDetector:
    def __init__(
        self,
        mode=False, compl=1, smoothLm=True, smoothSg=True, seg=False, detectionCon=0.5, trackCon=0.5):
        self.mode = mode
        self.compl = compl
    ...
    def find_angle(self, img, bodypart1, bodypart2, bodypart3, draw=True):
        ...
        if draw:
            ...
            cv2.putText(img, str(int(angle)), (bx - 50, by + 50),
    ...
    def check_exercise_progress(self, img, bodypart1, bodypart2, bodypart3, startang, endang, draw=True):
        ...
        if len(self.landmarks) !=0:
            ...
            per = int(np.interp(angle, (startang, endang), (0, 100)))
...
def live_demo():
    ...
    while True:
        succ, frame = cap.read()
        detector.check_exercise_progress(frame, BodyPart.RIGHT_SHOULDER.value, BodyPart.RIGHT_ELBOW.value, BodyPart.RIGHT_WRIST.value, 50, 160)
